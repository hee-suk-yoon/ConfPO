...
if is_peft_available():
    sys.path.append(
        os.path.abspath(os.path.join(os.path.dirname(__file__), os.path.pardir, os.path.pardir, os.path.pardir, 'peft-0.11.1', 'src')) # transformers directory
    )
...
# added by esyoon 2024-07-13-17:27:37
@dataclass
class PPODataCollatorWithPadding:
    r"""
    DPO DataCollator class that pads the tokenized inputs to the maximum length of the batch.
    Args:
        pad_token_id (`int` defaults to 0):
...
def generate(
    ...
    output = lm_backbone.generate(
        ...
        synced_gpus=True, # added by esyoon 2024-07-17-15:28:00
        output_scores=True,
    )
    logits = torch.stack(output.scores, 1)
    return torch.cat((queries, output.sequences[:, context_length:]), dim=1), logits

def generate_hsy( #added by HSY 7/18/24 TODO delete
    lm_backbone: torch.nn.Module, queries: torch.Tensor, pad_token_id: int, generation_config: dict
) -> Tuple[torch.Tensor, torch.Tensor]:
    """
    Generates sequences from the language model backbone in a way that does not affect padding tokens.
    ...
    attention_mask = queries != pad_token_id
    input_ids = torch.masked_fill(queries, ~attention_mask, 0)
    #with lm_backbone.summon_full_params():
    with lm_backbone.disable_adapter():
        output = lm_backbone.generate(
            input_ids=input_ids, 
            attention_mask=attention_mask,
            # position_ids=attention_mask.cumsum(1) - attention_mask.long(), # not needed: already adjusted in generations
